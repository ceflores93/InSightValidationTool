...
namespace Cognex.InSight.Web.Controls
{
  public partial class CvsSpreadsheet : UserControl
  {
    ...
    public CvsSpreadsheet(bool isCustomView)
    {
      ...
      InitializeCustomViewComponent();
    }

    public void setCustomViewName(CvsInSight insight)
    {
      _inSight = insight;
      if (insight != null && insight.CustomViewSettings != null && insight.CustomViewSettings.Length > 0)
      {
        HmiCustomViewSettings cvSettings = insight.CustomViewSettings[0];
        customViewName.Text = cvSettings?.Name;
      }
    }
